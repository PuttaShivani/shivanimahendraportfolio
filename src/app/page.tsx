...
const Footer = dynamic(() => import(/* webpackPrefetch: true */ "@/components/layout/Footer").then(mod => mod.Footer), {
    ssr: false
});

const Hyperspeed = dynamic(() => import(/* webpackPrefetch: true */ '@/components/ui/Hyperspeed'), { ssr: false });
const { hyperspeedPresets } = require('@/components/ui/Hyperspeed');

const Scene3D = dynamic(() => import(/* webpackPrefetch: true */ '@/components/three/Scene3D').then(mod => ({ default: mod.Scene3D })), {
    ssr: false,
    loading: () => null
});

const AboutSection = dynamic(() => import(/* webpackPrefetch: true */ "@/components/sections/AboutSection"), {
    ssr: false,
    loading: () => <div className="h-[600px] w-full animate-pulse bg-zinc-100/5 dark:bg-zinc-800/5" />
});

const ExpertiseSection = dynamic(() => import(/* webpackPrefetch: true */ "@/components/sections/ExpertiseSection"), {
    ssr: false,
    loading: () => <div className="h-[500px] w-full animate-pulse bg-zinc-100/5 dark:bg-zinc-800/5" />
});

const HeroVisual = dynamic(() => import(/* webpackPrefetch: true */ "@/components/sections/HeroVisual").then(mod => mod.HeroVisual), {
    ssr: false
});
...
const CTASection = dynamic(() => import(/* webpackPrefetch: true */ "@/components/sections/CTASection"), {
    ssr: false,
...
export default function HomePage() {
    ...
    const handleLoadingComplete = () => {
        ...
        // Cascade refreshes: catch sections that mount progressively
        const refreshAll = () => {
            window.dispatchEvent(new Event('resize'));
            ScrollTrigger.refresh();
        };
    ...
    return (
        ...
        className={`relative overflow-x-clip will-change-transform will-change-opacity${isLoading ? ' pointer-events-none' : ''}`}
            >
                {/* 
                ...
                */}
                ...
                {/* Pre-mount all sections so ScrollTrigger has correct measurements
                    from the start. They remain invisible while the loader is active
                    because the parent motion.main is at opacity:0. */}
                <ExpertiseSection />
                <AboutSection />
                <MetricCTAHijack />
